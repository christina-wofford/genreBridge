...
def search():#Search for the book and author entered by the user
    ...
    #Pass the results back to the front end so that the spotify player show the returned playlist
...
def plot_page_count_vs_track_count(): #Figure #1: Correlation between book length and playlist length
    # Fetch data from the database
    cursor.execute("""
        SELECT b.page_count, COUNT(ptm.track_id) as track_count
        FROM books b
        JOIN playlist_book_mapping pbm ON b.book_id = pbm.book_id
        JOIN playlist_track_mapping ptm ON pbm.playlist_id = ptm.playlist_id
    ...
    page_counts = [row[0] for row in data]
    track_counts = [row[1] for row in data]

    # Create a scatter plot
    plt.scatter(page_counts, track_counts)
    plt.xlabel("Page Count")
    plt.ylabel("Track Count")
    plt.title("Page Count vs Track Count")
    plt.show()
